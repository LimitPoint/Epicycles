...
class ThumbnailProvider: QLThumbnailProvider {
    ...
    override func provideThumbnail(for request: QLFileThumbnailRequest, _ handler: @escaping (QLThumbnailReply?, Error?) -> Void) {
        ...
        handler(QLThumbnailReply(contextSize: thumbnailFrame.size, drawing: { (context) -> Bool in
            ...
            #if os(macOS)
            DrawPathsInContext(context: context, paths: [curvePath], width: Int(viewSize.width), height: Int(viewSize.height), lineWidth: [1], lineColor: [lineColor[0]], backgroundColor: nil, pathScaleFactor: 1, flipCGContext: true)
            #else
            DrawPathsInContext(context: context, paths: [curvePath], width: Int(viewSize.width), height: Int(viewSize.height), lineWidth: [1], lineColor: [lineColor[0]], backgroundColor: nil, pathScaleFactor: 1, flipCGContext: false)
            #endif
